...
stat_bundesamt_jung <- read_excel(file.path(path, "Vorhersage2060jung.xls"),
                                  sheet = 1,
                                  skip = 3,
                                  col_names = FALSE)
...
stat_bundesamt_jung <- stat_bundesamt_jung[,1:2]
colnames(stat_bundesamt_jung) <- c("Female_2060", "Male_2060")
stat_bundesamt_jung$AGE <- 0:99
...
for (i in seq_len(m)) {
  male_survival_rate <- male_life_table$p[1:(n - 1)] + i * 0.000047
  male_fertility_rate <- rep(x = 0, times = n)
  
  female_survival_rate <- female_life_table$p[1:(n - 1)] + i * 0.000037
  female_fertility_rate <- c(rep(x = 0, times = 14), (fertility_table$fertility_rate + 0.4225 * min(i, 14) ) / 1000, rep(x = 0, times = n - 49))
  ...
  life_expectancy_by_year[i] <- life_expectancy(0, male_life_table$p[1:(n - 1)] + i * 0.000047)
  ...
  german_pop[female_next] <- german_pop[female_next] + 
                            c(rep(7000 / 20, 20), rep(25000 / 20, 20), rep(0, 25), rep(-3000 / 35, 35)) + 57 * min(i, 9)
                             
  
  german_pop[male_next] <- german_pop[male_next] + 
                            c(rep(4000 / 20, 20), rep(18000 / 20, 20), rep(-5000 / 25, 25), rep(-5000 / 35, 35)) + 57 * min(i, 9)
...
stat_bundesamt_jung <- gather(data = stat_bundesamt_jung , YEAR, POP, -AGE)
stat_bundesamt_jung  <- separate(stat_bundesamt_jung, col = YEAR, into = c("SEX", "YEAR"), sep = "_")
stat_bundesamt_jung[stat_bundesamt_jung$SEX == "Male",]$POP <- -stat_bundesamt_jung[stat_bundesamt_jung$SEX == "Male",]$POP 
...
q <- ggplot(stat_bundesamt_jung, aes(x = AGE, y = POP, fill = SEX, width = 1)) +
      coord_flip() +
      geom_bar(data = stat_bundesamt_jung[stat_bundesamt_jung$SEX == "Female",], stat = "identity") +
      geom_bar(data = stat_bundesamt_jung[stat_bundesamt_jung$SEX == "Male",], stat = "identity") 
print(q)

```

# Szenario 1 - Beschreibung
Die Sterberaten wurden durch die Addition eines konstanten Faktors erhöht. Bis zum Jahr 2065 wurde dieser Faktor so skaliert, dass die Lebenserwartung für Frauen 89.2 Jahre und für Männer 85 Jahre beträgt. Der Faktor für die Männer ist 0.000047 und der Faktor für die Frauen ist 0.000037. Analog wurde die Geburtenrate modifiziert, sodass im Jahr 2025 eine Geburtenhäufigkeit von 1.6 Kindern pro Frau vorliegt. Die Migration findet durch die Addtion eines Vektors statt, der nach verschiedenen Altersgruppen aufgeteilt ist. Um einen Anstieg der Migration zu simulieren wurde jeder Eintrag bis 2020 durch einen konstanten Faktor erhöht.
Bei der ersten Betrachtung der beiden Bevölkerungspyramiden fällt auf, dass bei unserer Vorherage eine Lücke in den Daten vorliegt. Die Zensus-Daten aus dem Jahr 2011 geben den Bevökerungsstand nicht korrekt wieder. Eine Fehlerquelle liegt weiterhin in der gleichmäßigen Erhöhung der Überlebensrate über alle Altersstufen hinweg. Es wird somit nicht eingefangen, dass sich die aufgrund des medizinischen Fortschritts vorallem auf die älteren Generationen beschränkt.
In beiden Grafiken ist der Übergang zur Urenenform erkennbar.